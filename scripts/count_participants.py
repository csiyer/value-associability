...
Usage: python scripts/count_participants.py
...
def count_participants_task(path):
    ...
    if old_trials_df.empty:
        AT_CHANCE_PIDS = []
    else:
        binom_df = old_trials_df.groupby('participant_id').apply(subject_sig_above_chance, col=chance_col, include_groups=False).reset_index()
        AT_CHANCE_PIDS = binom_df.participant_id[~binom_df[0]].unique().tolist()
    print(f"     {len(AT_CHANCE_PIDS)} failed to pass binomial test from chance")
