...
# sync new raw participant files from OSF into data/mixed_memorability/ (conflicts = "skip" never overwrites/re-downloads local files)
osf_retrieve_node("8d2cb") |>
  osf_ls_files(n_max = Inf) |>
  filter(name == "mixed_memorability") |>
  osf_download(path = data_dir, recurse = TRUE, conflicts = "skip")

episodic_choice_data <- read_csv(file.path(data_dir, "episodic_choice_data-mixed_memorability.csv"))
